...
namespace AG_Interface
{
	class ArrayGraphicsInterface
	{
		...
		public byte[] RASTER_BUFFER_CREATE(byte NumBuffers, int BufferSize, byte PrintDirFwd, byte HorizontalDPIEnum, byte NumPens, int NumberOfLongWordsInLine, int NumberOfLinesInImage, int HorizontalDpi, bool nBufferPrefill)
		{
			...
			msg.ucNumBuffers = NumBuffers;
			msg.uliBufferSize = (uint)IPAddress.HostToNetworkOrder(BufferSize);
			msg.ucPrintDirFwdRev = PrintDirFwd;
			msg.ucHorizontalDpiEnum = HorizontalDPIEnum;
			msg.ucNumPens = NumPens;
			msg.uiNumberOfLongWordsInLine = (uint)IPAddress.HostToNetworkOrder(NumberOfLongWordsInLine);
			msg.uiNumberOfLinesInImage = (uint)IPAddress.HostToNetworkOrder(NumberOfLinesInImage);
			msg.uiHorizontalDpi = (uint)IPAddress.HostToNetworkOrder(HorizontalDpi);
		...
		public unsafe bool FillBufferFromImage(double lane, int buffernr)
		{
					...
					if ((y >= 0) && (y < FullBitmapHeight))
					{
						for (int j = 0; j < BufferPixelDataColumcount; j++)
						{
							...
							if ((x >= 0) && (x < FullBitmapWidth))
							{
								...
								byte pixelR = row[x];
								if (PalleteEntryIsBlack[pixelR]) Setpixel(j, i, BufferPixelData);
							}
							if ((y % 150) == 0x00) Setpixel(j, i, BufferPixelData);    //debug
							if ((x % 150) == 0x00) Setpixel(j, i, BufferPixelData);    //debug                      
						}
					}
				}
		...
		internal void DirectPrintBufferNoWait(ListBox statuslistbox)
		{
			...
			for (col = 0; col < 150; col += 16) //512 bits per vert row
			{
				...
				{
					BufferPixelData[i + (col * columnheightbytesize)] = 255; //dummy data
				}
			}
		...
		public void UploadLaneNoWait(double lane)
		{
		...
		internal void FillNextPrintBufferRLE()
		{
			for (int i = 0; i < BufferPixelData.Length; i++) BufferPixelData[i] = 0; //white all 
			bool printabledata = true;
			//    if ((CurrentBufferNumber % Buffers) == 0) 
			FillBufferFromImage(CurrentLaneNumber, 0/* CurrentBufferNumber % Buffers*/); //test; 2nd and up only with mage data
